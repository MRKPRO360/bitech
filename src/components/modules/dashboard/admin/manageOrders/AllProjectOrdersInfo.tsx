'use client';

import { IOrder } from '@/types';
import Image from 'next/image';
import Link from 'next/link';
import { Frown, ChevronDown } from 'lucide-react';
import { IMeta } from '@/types/meta';
import formateDate from '@/components/utils/formateDate';
import TablePagination from '@/components/ui/core/TablePagination';
import { useState } from 'react';
import {
  revalidateOrders,
  updateCustomerOrdersStatus,
} from '@/services/orders';
import { toast } from 'sonner';

const STATUS_OPTIONS = ['Pending', 'Completed', 'Cancelled'] as const;

const AllProjectOrdersInfo = ({
  orders,
  meta,
}: {
  orders: IOrder[];
  meta: IMeta;
}) => {
  const [openDropdown, setOpenDropdown] = useState<string | null>(null);

  const handleStatusChange = async (orderId: string, status: string) => {
    console.log('Updating order:', orderId, '→', status);
    const toastId = toast.loading('Updating order status...');

    try {
      const res = await updateCustomerOrdersStatus(orderId, status);

      console.log(res);

      if (res.success) {
        revalidateOrders();
        toast.success(`Status updated to ${status}`, {
          id: toastId,
        });
      } else {
        toast.error(res.message);
      }
    } catch (err: any) {
      console.log(err);
      toast.error(err.message || 'Failed to update status', {
        id: toastId,
      });
    }
  };

  return (
    <div className="min-h-screen mt-14 mb-4 space-y-6">
      {orders.length === 0 ? (
        <div className="flex text-lg font-semibold items-center justify-center text-gray-500 gap-2 w-full h-full">
          <Frown />
          <span>No project order available.</span>
        </div>
      ) : (
        orders.map((order, index) => (
          <div
            key={index}
            className="p-4 shadow-md hover:shadow-lg rounded-lg bg-white"
          >
            <h2 className="text-xl font-semibold mb-4">
              Order Issued: {formateDate(order.createdAt as string)}
            </h2>

            <div className="space-y-3">
              {order.projects.map((item) => (
                <div
                  key={item.project._id}
                  className="flex justify-between items-center p-3 rounded-md transition duration-300"
                >
                  <Link
                    href={`/prebuiltProjects/${item.project._id}`}
                    className="flex items-center gap-4 font-semibold hover:underline hover:text-green-700"
                  >
                    <Image
                      width={56}
                      height={56}
                      src={item.project.images?.thumbnail as string}
                      alt={item.project.shortDescription}
                      className="w-14 h-14 rounded-md object-cover"
                    />
                    <p>{item.project.slug}</p>
                  </Link>

                  <div className="flex gap-6 items-center">
                    <p>
                      <strong>Amount:</strong> <span>{order.amount}</span>
                    </p>

                    <p>
                      <strong>Payment Method:</strong>{' '}
                      <span>{order.paymentMethod}</span>
                    </p>

                    <div className="relative">
                      <p className="flex items-center gap-2">
                        <strong>Status:</strong>
                        <span
                          className={`px-2 py-1 rounded text-sm font-semibold 
                            ${
                              order.orderStatus === 'Pending'
                                ? 'bg-yellow-100 text-yellow-700'
                                : ''
                            }
                            ${
                              order.orderStatus === 'Completed'
                                ? 'bg-green-100 text-green-700'
                                : ''
                            }
                            ${
                              order.orderStatus === 'Cancelled'
                                ? 'bg-red-100 text-red-700'
                                : ''
                            }
                          `}
                        >
                          {order.orderStatus}
                        </span>

                        {/* Show button only if not completed */}
                        {order.orderStatus !== 'Completed' && (
                          <button
                            onClick={() =>
                              setOpenDropdown(
                                openDropdown === order._id ? null : order._id
                              )
                            }
                            className="p-1 rounded hover:bg-gray-100"
                          >
                            <ChevronDown className="w-4 h-4" />
                          </button>
                        )}
                      </p>

                      {/* Dropdown menu */}
                      {openDropdown === order._id && (
                        <div className="absolute right-0 mt-2 w-40 bg-white  rounded-md shadow-lg z-10">
                          {STATUS_OPTIONS.map((status) => (
                            <button
                              key={status}
                              onClick={() => {
                                handleStatusChange(order._id, status);
                                setOpenDropdown(null);
                              }}
                              className={`w-full text-left px-4 py-2 text-sm cursor-pointer  
                            ${
                              status === 'Completed'
                                ? 'bg-green-100 hover:bg-green-200 text-green-700'
                                : ''
                            }
                            ${
                              status === 'Cancelled'
                                ? 'bg-red-100 hover:bg-red-200 text-red-700'
                                : ''
                            } ${
                                status === order.orderStatus
                                  ? 'text-gray-400 cursor-not-allowed'
                                  : ''
                              }`}
                              disabled={status === order.orderStatus}
                            >
                              {status}
                            </button>
                          ))}
                        </div>
                      )}
                    </div>
                  </div>
                </div>
              ))}
            </div>
          </div>
        ))
      )}
      <TablePagination totalPage={meta?.totalPage} />
    </div>
  );
};

export default AllProjectOrdersInfo;
