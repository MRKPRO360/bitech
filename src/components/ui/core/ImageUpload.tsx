...
function ImageUpload({
  ...
  return (
    <div>
      <label className=" block mb-1 text-base font-semibold text-gray-600">
        {label}
      </label>
        ...
        <div className="grid grid-cols-3 gap-4 my-4">
          {previews.map((src, idx) => (
            <div className="relative" key={idx}>
              ...
              />
              <button
                className="top-1 right-1 hover:bg-red-600 hover:text-white   absolute p-1 transition duration-300 bg-white rounded-full shadow"
                type="button"
                onClick={(e) => {
                  e.stopPropagation();
                  handleRemove(idx);
